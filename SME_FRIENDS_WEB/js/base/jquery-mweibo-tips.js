...
(function($){
		...
		return this.each(function(){
			...
			$(this).on("mouseover", function (e) {
				...
				if(currentObj && currentObj == this){			
				    ...
				    if (opts.SetConent != undefined && opts.SetConent != null && $(this).data("HasLoaded") != true) {//对当前对象只加载一次
				        opts.SetConent.call(this, opts.CreaterUserType, opts.CreaterID, opts.CreaterName);
				        $(this).data("HasLoaded", true);
				        $("img[id^='imgUser'][id!='" + $(this).attr("id") + "']").removeData("HasLoaded");//移除其他的加载状态，查看其他的时候重新加载
				    }
					$wrap.show();
				} else {
					currentObj = this;	
					...
					$wrap.off('mouseover').on('mouseover', function (e) {
						e.stopPropagation();
						_clearTimer();
					});
	...
	$.fn.pinwheel.defaults = {
	    SetConent: function () { console.log("set content here")}
	};
})(jQuery);