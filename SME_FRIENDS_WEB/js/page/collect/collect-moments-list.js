Collect.Moments = function () { }
Collect.Moments.registerClass("Collect.Moments");

Collect.Moments.Init = function init() {
    Collect.Moments.Search({ pageStart: 1, pageEnd: Collect.PageSize * 1 });
    //搜索框回车事件绑定
    ...
    //搜索我关注的包括我的微博
    $("#spnSearch").off("click").on("click", { Page: { pageStart: 1, pageEnd: Collect.PageSize * 1 } }, Collect.Moments.SearchEvent);
}

//搜索博文事件
Collect.Moments.SearchEvent = function SearchEvent(event) {
    var page = event.data.Page;
    Collect.Moments.Search(page);
}

//分页搜索
Collect.Moments.Search = function search(page) {
    Collect.Moments.SearchBind(page);
    var keyword_view = {
        Keyword: $("#txtKeyword").val()
    };
    $.SimpleAjaxPost("service/MomentsService.asmx/GetCollectInfoCount", true,
...
//搜索我关注的用户（包括我的）的博文
Collect.Moments.SearchBind = function search_bind(page) {
    var keyword_view = {
        Keyword: $("#txtKeyword").val()
    };
    $("#divCollectEmpty").hide();
    $.SimpleAjaxPost("service/MomentsService.asmx/GetCollectInfos", true,
                         ...
                         function (json) {
                             ...
                             if (Array.isArray(result) && result.length != 0) {
                                 ...
                                 $.each(result, function (index, item) {
                                     ...
                                     $(document).off("click", "#aShowDetailLong" + index).on("click", "#aShowDetailLong" + index, { LongObj: item }, Collect.Moments.ShowDetailLongEvent);
                                 ...
                                 $.each(result, function (index, item) {
                                     //取消收藏
                                     $("#liCancelCollect" + index).on("click", { CollectID: item.ID }, Collect.Moments.DeleteCollectEvent);
                                 });
                             } else {
                                 $("#ulCollectList").empty();
...
Collect.Moments.DeleteCollectEvent = function DeleteCollectEvent(event) {
    ...
    $.Confirm({
        ...
        $.SimpleAjaxPost("service/MomentsService.asmx/CancelCollect",
             true,
             "{collectID:'" + id + "'}",
              function (json) {
                  var result = json.d;
                  if (result == true) {
                      ...
                      console.log("Collect.Moments.DeleteCollectEvent:取消失败！");
                  }
              });
    });
}
...
//长篇博文展示
Collect.Moments.ShowDetailLongEvent = function ShowDetailLongEvent(event) {
