...
/// </summary>
public partial class MomentsService
{
    private static readonly IBehavior<CollectInfo> IcollectInfo = new CollectInfoDao();
    private static readonly IBehavior<PraiseInfo> IpraiseInfo = new PraiseInfoDao();
    private static readonly IBehavior<BrowseInfo> IbrowseInfo = new BrowseInfoDao();
    private static readonly IBehavior<TreadInfo> ItreadInfo = new TreadInfoDao();
    private static readonly IBehavior<ReportInfo> IreportInfo = new ReportInfoDao();
    private static readonly IBehavior<CommentInfo> IcommentInfo = new CommentInfoDao();
    ...
    [WebMethod(BufferResponse = true, Description = "点踩或取消踩")]
    [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
    [GenerateScriptType(typeof(BoolWithPrimaryKeyView))]
    ...
    [WebMethod(BufferResponse = true, Description = "举报")]
    [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
    [GenerateScriptType(typeof(BoolWithPrimaryKeyView))]
    ...
    public string GetCollectInfos(MyKeywordView keywordView, MiicPage page)
    {
        ...
        if (dt.Rows.Count > 0)
        {
            var temp = from dr in dt.AsEnumerable()
                       ...
                       new
                       {
                           ID = dr["MomentsCollectInfo" + Config.Attribute.GetSqlColumnNameByPropertyName<CollectInfo, string>(o => o.ID)].ToString(),
                       ...
                       select new
                       {
                           ...
                           AccInfos = (from item in g
                                       ...
                                       select new
                                       {
                                           MicroAccessoryInfoID = item["MomentsAccessoryInfo" + Config.Attribute.GetSqlColumnNameByPropertyName<AccessoryInfo, string>(o => o.ID)].ToString(),
    ...
    [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
    [GenerateScriptType(typeof(CommentSearchView))]
    [GenerateScriptType(typeof(MiicPage))]
